    ...
    <div className="relative">
      ...
      >
        ...
        {events.map((event) => {
          ...
          const color =
            EVENT_COLORS[event.event_type] || '#64748b';

          const radius =
            SEVERITY_SIZES[event.severity] || 6;
              ...
              <Popup className="custom-popup">
                <div className="min-w-[250px] p-2">
                  ...
                  <div className="grid grid-cols-2 gap-1 text-xs">
                    ...
                    {event.category_confidence != null && (
                      <div className="col-span-2">
                        <span className="font-medium">
                          Classification Confidence:
                        </span>{' '}
                        {(event.category_confidence * 100).toFixed(0)}%
                      </div>
                    )}

                    {event.fake_confidence != null && (
                      <div className="col-span-2">
                        <span className="font-medium">
                          Misinformation Risk:
                        </span>{' '}
                        {(event.fake_confidence * 100).toFixed(0)}%
                      </div>
                    )}

                    <div className="col-span-2 text-gray-400">
                      {dayjs(event.reported_at).format(
                        'DD MMM YYYY, hh:mm A'
                      )}
                    </div>
                  </div>
      ...
      {showLegend && (
        <div className="absolute bottom-4 left-4 z-[1000] bg-dark-900/95 backdrop-blur-sm border border-dark-700/50 rounded-lg p-3">
          ...
          <p className="mt-2 border-t border-dark-700/50 pt-2 text-[10px] text-gray-500">
            Ring indicates verified events
          </p>
        </div>
      )}
    </div>
