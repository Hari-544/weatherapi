...
from app.models.weather_event import WeatherEvent, EventType, SeverityLevel, VerificationStatus, EventSource
...
async def events_by_type(
    ...
    return {
        "data": [
            {"event_type": r.event_type.value if r.event_type else None, "count": r.count}
            for r in rows
        ]
    }
...
async def events_by_state(
    ...
    query = query.group_by(WeatherEvent.state).order_by(func.count(WeatherEvent.id).desc())
    result = await db.execute(query)
    rows = result.all()
    return {
        "data": [
...
async def events_over_time(
    ...
    if granularity == "month":
        ...
        by = [extract('year', WeatherEvent.reported_at), extract('month', WeatherEvent.reported_at), extract('day', WeatherEvent.reported_at)]
    ...
    for r in rows:
        if granularity == "day":
            ...
            label = f"{int(r.year)}-W{int(list(r[1:])[0]) if len(r[1:]) > 0 else 0}"
...
async def verification_stats(db: AsyncSession = Depends(get_db)):
    ...
    return {
        "data": [
            {"verification_status": r.verification_status.value if r.verification_status else None, "count": r.count}
            for r in rows
        ]
    }
...
async def severity_distribution(db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(
            ...
            func.count(WeatherEvent.id).label("count")
        ).group_by(WeatherEvent.severity).order_by(WeatherEvent.severity)
    )
    rows = result.all()
    return {
        "data": [
            {"severity": r.severity.value if r.severity else None, "count": r.count}
            for r in rows
        ]
    }
...
async def recent_events(
    ...
    result = await db.execute(
        select(WeatherEvent).order_by(WeatherEvent.reported_at.desc()).limit(limit)
    )
...
async def source_breakdown(db: AsyncSession = Depends(get_db)):
    ...
    return {
        "data": [
            {"source": r.source.value if r.source else None, "count": r.count}
            for r in rows
        ]
    }